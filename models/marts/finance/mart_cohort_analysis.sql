...
customer_order_history as (
    select 
        ...
        -- Months since customer's first order
        datediff('month', f.first_order_date, p.order_date) as period_number,
        
        -- Running total of revenue and quantity
        sum(p.revenue) over (
        ...
        -- Sequential order number for each customer
        row_number() over (
            partition by p.order_id, p.customer_segment, p.partner_channel
            order by p.order_date
        ) as order_sequence
...
cohort_analysis as (
    select 
        ...
        -- Initial cohort size and revenue
        s.cohort_size,
        ...
        -- Revenue retention and per-customer ratio
        ...
        -- High-value and repeat customer rates
...
cohort_insights as (
    select 
        *,
        ...
        -- Assessed business value of the cohort
        case 
...
select 
    ...
    *,
    ...
    -- Recommended investment level for the cohort
    case 
