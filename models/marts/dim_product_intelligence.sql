...
with product_base as (
    select 
        product_sku,
        max(coalesce(product_category, 'Unknown')) as product_category,
        ...
        -- Product sales timeline metrics
...
product_analytics as (
    select 
        ...
        -- Combined cost and expense data
...
product_classification as (
    select 
        *,
        row_number() over (partition by product_sku order by total_revenue desc) as rn,
        ...
        -- Current product lifecycle stage
        case 
...
select 
    ...
    *,
    ...
    -- Calculated product performance score
    ...
    -- Recommended next action for the product
    case 
